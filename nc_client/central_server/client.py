...
class CentralServerClient:
    ...
    def set_connection_sharing(self, ip_address, port, is_shared):
        """Set sharing status for a server connection - CLIENT VERSION (sends command to server)"""
        if not self.authenticated:
            return False, "Not authenticated"
    ...
    def get_server_connection_info(self, ip_address, port):
        ...
        # DON'T send current username to update recent connection
        # Just request the current info without modifying it
        response = self.send_command("get_server_connection_info", {
            "ip_address": ip_address,
            "port": port
            # Removed username parameter to avoid updating the record
        })
