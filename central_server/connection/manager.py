...
class ConnectionManager:
    ...
    def handle_register_server(self, data, address):
        ...
        if success:
            # Now update the sharing_with field with the correct username
            try:
                conn = self.database.get_connection()
                cursor = conn.cursor()
                ...
                if server_record:
                    ...
                    # Update the sharing_with field with the CURRENT user (not admin)
                    cursor.execute(
                        "UPDATE servers SET sharing_with = ? WHERE server_id = ?",
                        (username, server_id)
                    )
                    conn.commit()
                    logging.info(f"Updated sharing_with to '{username}' for server {ip_address}:{port}")
    ...
    def handle_get_server_connection_info(self, data, address):
        ...
        try:
            ...
            server_id = server_record['server_id']
            last_seen = server_record['last_seen']
            current_sharing_with = server_record['sharing_with'] if 'sharing_with' in server_record else None

            # Only update if this is actually a connection request (not just viewing)
            # We can tell this by checking if the requesting username is different from current sharing_with
            connection_info = []

            # Always show the current sharing_with as the recent connection
            if current_sharing_with:
                connection_info.append({
                    'server_ip': ip_address,
                    'recent_connection': current_sharing_with,
                    'connected_at': last_seen
                })
            else:
                # If no current sharing, show as no recent connection
                connection_info.append({
                    'server_ip': ip_address,
